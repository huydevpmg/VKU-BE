...
// Tạo một biến khóa để kiểm soát truy cập đồng thời từ nhiều máy
let isSearching = false;

app.get("/search", async (req, res) => {
  // Kiểm tra xem có đang tìm kiếm hay không, nếu có thì trả về lỗi
  if (isSearching) {
    return res.status(429).send("Too Many Requests: Only one search is allowed at a time");
  }

  // Đặt cờ isSearching thành true để chỉ ra rằng đang thực hiện tìm kiếm
  isSearching = true;
  ...
  try {
    ...
    // Kiểm tra xem có tham số tìm kiếm không hoặc không có dữ liệu
    ...
    // Chỉ trả về kết quả đầu tiên nếu có
    const searchResult = filteredItems.length > 0 ? [filteredItems[0]] : [];
    ...
    // Đặt cờ isSearching lại thành false sau khi tìm kiếm hoàn tất
    isSearching = false;
  }
});
